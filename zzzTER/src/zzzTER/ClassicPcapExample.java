...
public class ClassicPcapExample {
	...
	public static void main(String[] args) throws Exception{
		...
		System.out
		    .printf("\nChoosing '%s' [%s] :\n",
		        (device.getDescription() != null) ? device.getDescription()
		            : device.getName(),asString(device.getHardwareAddress()));
		...
		PcapPacketHandler<String> jpacketHandler = new PcapPacketHandler<String>() {
			...
			public void nextPacket(PcapPacket packet, String user) {
				...
				// Source
				byte[] ipps = packet.getHeader(ip).source();
				byte[] eths= packet.getHeader(ethernet).source();
				
				// destination 
				
				byte[] ippd = packet.getHeader(ip).destination();
				byte[] ethd= packet.getHeader(ethernet).destination();
				
				
				System.out.printf("Received packet at %s \t caplen=%-4d \t len=%-4d \t user %s  \t source %s    \t destination %s \t source %s \t destination %s\n",
				    new Date(packet.getCaptureHeader().timestampInMillis()), 
				    packet.getCaptureHeader().caplen(),  // Length actually captured
				    packet.getCaptureHeader().wirelen(), // Original length 
				    user, (packet.hasHeader(ip)==true) ? asIp(ipps) : "##", (packet.hasHeader(ip)==true) ? asIp(ippd) : "##" ,(packet.hasHeader(ethernet)==true) ? asString(eths) : "##"  // User supplied object
				  ,  (packet.hasHeader(ethernet)==true) ? asString(ethd) : "##" );
			}
		};
...
//		}  
		 ...
		 **************************************************************************/
		pcap.loop(/* pcap.LOOP_INFINITE */10, jpacketHandler, "Tarik");
